...
describe('parseProductId', () => {
  ...
  test('lève une erreur pour un id invalide', () => {
    expect(() => parseProductId('abc')).toThrow(
...
describeIfDatabase('Tests d’intégration produits avec PostgreSQL', () => {
  beforeAll(async () => {
    await db.query(
      'CREATE TABLE IF NOT EXISTS products (id SERIAL PRIMARY KEY, name TEXT NOT NULL, description TEXT NOT NULL, price_cents INTEGER NOT NULL CHECK (price_cents > 0));'
    );
    await db.query(
      "INSERT INTO products (name, description, price_cents) VALUES ('Test Product', 'Test description', 1000) ON CONFLICT DO NOTHING;"
    );
  });
